...
router.post("/addNewCohort", (req, res) => {
  const createCohortQuery =
    "INSERT INTO cohort (name, start_date , description, state_level_organization_ref_id) VALUES ($1, $2, $3, $4) RETURNING cohort_id";

  const addtoCohortRequirementQuery =
    "INSERT INTO cohort_requirements (cohort_id, requirement_id, due_date, notification_1_date, notification_2_date, cycle) VALUES ($1, $2, $3, $4 , $5, $6 )";

  pool
    .query(createCohortQuery, [
      req.body.newCohort.name,
      req.body.chosenDate,
      req.body.newCohort.note,
      req.body.newCohort.state_level_organization
    ])
    .then(result => {
      ...
      req.body.newCohort.requirements.forEach(requirement => {
        pool
          .query(addtoCohortRequirementQuery, [
            ...
            requirement.notification_1_date,
            requirement.notification_2_date,
            1
          ])
          .then(result1 => {
...
router.post("/addNewSLO", (req, res) => {
  let queryText =
    "INSERT INTO state_level_organization (name, state) VALUES ($1, $2)";

  pool
    .query(queryText, [req.body.name, req.body.state])
    .then(response => {
      res.sendStatus(201);
    })
    .catch(err => {
...
router.get("/latestCohort/:state_level_org_id", (req, res) => {
  let queryText =
    "SELECT * FROM cohort WHERE state_level_organization_ref_id = $1 ORDER BY cohort_id DESC LIMIT 1";

  pool
    .query(queryText, [req.params.state_level_org_id])
    .then(response => {
...
router.get("/cohortById/:cohort_id", (req, res) => {
  ...
  let queryText = `SELECT *, cohort.name as cohort_name, cohort.description as c_desc, requirements.name as req_name FROM cohort
    ...
    WHERE cohort.cohort_id = $1 ORDER BY cycle`;

  pool
    .query(queryText, [req.params.cohort_id])
    .then(response => {
      console.log(response.rows);
      let responseObj = {
      ...
      response.rows.forEach(row => {
        responseObj.cycle = row.cycle;

        if (!responseObj.requirements.hasOwnProperty(row.cycle)) {
          ...
          responseObj.requirements[row.cycle].push({
            requirement_id: row.requirement_id,
            requirement_name: row.req_name,
            due_date: row.due_date,
            notification_1_date: row.notification_1_date,
...
router.put("/updateById/:cohort_id", (req, res) => {
  ...
  let updateCohortQuery =
    "UPDATE cohort SET name = $1, description = $2, state_level_organization_ref_id = $3 WHERE cohort_id = $4";

  let updateCohortRequirements =
    "UPDATE cohort_requirements SET due_date = $1, notification_1_date = $2, notification_2_date = $3 WHERE cohort_id = $4 AND requirement_id = $5 AND cycle = $6";


  pool
    .query(updateCohortQuery, [
      req.body.name,
      req.body.note,
      req.body.state_level_organization,
    ...
    .then(response => {
      ...
      for(let cycle in req.body.requirements){
          allRequirements = [...allRequirements, ...req.body.requirements[cycle]]
      }

      let reqCounter = allRequirements.length;
      

      allRequirements.forEach(requirement => {
        pool
          .query(updateCohortRequirements, [
            requirement.due_date,
            requirement.notification_1_date,
            requirement.notification_2_date,
            req.params.cohort_id,
